  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity" v-for="(item,index) in info.supports" :key="index">
          <div class="activity-item" :class="supportClasses[item.type]">
  ...
  export default {
    ...
    methods:{
      _initScroll(){
        ...
        //由于ui的高度等于父盒子高度,所以不能实现滚动,需要重新为ui动态赋值
        const UI=this.$refs.picUI
        const liWidth=120;
        const space=6
        const count=this.info.pics.length
        UI.style.width=(liWidth*space)*count-space+"px"
        new BScroll('.pic-wrapper', {//图片横向滚动
          scrollX:true
        })
      }
    },
    mounted() {
      // 如果数据还没有, 直接结束
      if(!this.info.pics) {//解决用户从其他页面过来或者在当前页面刷新报错的问题
        return
      }
